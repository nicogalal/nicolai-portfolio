...
- Third-party quotes: Iain Sawbridge (CCO, Podimo) and Fride von Porat (VP, Commercial Nordics, Podimo) are now placed together as a calm two-quote block in the homepage About section (directly after the leadership paragraph), both with final attributions. Still open: Karan Dang (Disney, Galaxy's Edge angle) for the Galaxy's Edge case, and a talent quote (e.g. from the Sports or Duellen creators) would be the strongest possible add. Keep each to 1-2 sentences, never a testimonial wall; target 3-4 voices total.
